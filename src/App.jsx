...
export default function App() {
  ...
  return (
    <div className="flex justify-center min-h-screen bg-[#0F172A] font-sans">
      <div className="w-full max-w-[430px] min-h-screen bg-[#1A1A2E] text-white flex flex-col relative">
        {/* 스크롤 영역 — 하단 nav 높이(80px)만큼 패딩 */}
        <div className="flex-1 overflow-y-auto overscroll-contain"
          style={{ paddingBottom: showCelebration ? '0px' : 'calc(80px + env(safe-area-inset-bottom))' }}>
          {renderMain()}
        </div>

        {/* ✅ FIX: fixed로 변경 — 항상 화면 하단에 고정, iOS safe area 대응 */}
        {!showCelebration && (
          <nav className="fixed bottom-0 left-0 right-0 bg-[#1A1A2E]/98 border-t border-white/10 flex justify-around items-center px-2 z-50"
