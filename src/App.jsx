  ...
  return (
    <div style={{
      ...
      <div style={{
        width: '100%',
        maxWidth: '430px',
        height: '100dvh',        /* Chrome/Safari 모두 주소창 포함 실제 높이 */
        display: 'flex',
        flexDirection: 'column',
        backgroundColor: '#1A1A2E',
        color: 'white',
        position: 'relative',
        overflow: 'hidden'       /* 컨테이너 밖으로 절대 안 나감 */
      }}>
        {/* 스크롤 영역 — flex-1로 남은 공간 채움, nav는 flex item으로 하단 고정 */}
        <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', WebkitOverflowScrolling: 'touch' }}>
          {renderMain()}
        </div>

        {/* ✅ nav를 flex item으로 — fixed/absolute 없이 항상 하단에 위치 */}
        {!showCelebration && (
          <nav style={{
            flexShrink: 0,
